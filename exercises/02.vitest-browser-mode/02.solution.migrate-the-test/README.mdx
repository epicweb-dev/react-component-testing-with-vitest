...
Usage-wise, both `render()` functions are nearly identical, where Vitest's version supports a subset of the render `options` compared to RTL. The one notable difference is that rendering in the browser is _asynchronous_, so I have to `await` the `render()` call:
...
Previously, I used the `screen` object from RTL to find elements in the virtual DOM. When using Vitest Browser Mode, I can now locate elements (and interact with them) using the `page` object from `vitest/browser` instead:
