...
A [project](https://vitest.dev/guide/projects.html) allows you to describe multiple configurations independently. Think of it as a package workspace, where a single repo can host multiple different packages. In the same way, a single Vitest process can run multiple different test projects.
...
```json highlight=6,10
{
	...
	"exclude": [],
	"compilerOptions": {
		"types": ["vitest/globals", "@vitest/browser/context", "@vitest/browser/matchers"]
	}
}
```
