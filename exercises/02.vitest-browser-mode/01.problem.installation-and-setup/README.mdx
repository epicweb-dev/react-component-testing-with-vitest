...
The last thing that remains is to make sure TypeScript sees the Vitest globals. Browser matcher types are included by default in Vitest 4, so there is nothing extra to add.
...
Finally, confirm that `tsconfig.test.json` only includes `vitest/globals` in `compilerOptions.types`:
