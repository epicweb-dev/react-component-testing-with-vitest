...
The last thing that remains is to make sure TypeScript sees the Vitest globals and the browser context types (these include the browser matchers).
...
Finally, confirm that `tsconfig.test.json` includes `vitest/globals` and `vitest/browser` in `compilerOptions.types`:
