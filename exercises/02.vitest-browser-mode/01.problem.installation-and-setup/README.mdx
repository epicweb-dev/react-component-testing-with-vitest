...
```sh nonumber
npm i -D @vitest/browser-preview vitest-browser-react
```

Let's break down what these packages do:

- [`@vitest/browser-preview`](https://www.npmjs.com/package/@vitest/browser-preview) is a _browser provider_ that runs your tests in a browser already installed on your machine. It is the quickest way to get started, and you will replace it with a proper provider later in this workshop;
...
As the next step, I need to tell Vitest which browser provider to use and what browsers I want for testing my components. Let's start with the `preview` provider and Chromium by setting `test.browser.provider` and `test.browser.instances`:

```ts filename=vite.config.ts add=3,12-17
/// <reference types="vitest/config" />
import { defineConfig } from 'vite'
...
The last thing that remains is to extend the TypeScript configuration to recognize DOM-specific matchers, like `expect().toBeVisible()`, as they come built-in in Vitest Browser Mode.
...
And replace it with a type reference to Vitest's browser types in `tsconfig.test.json`:
...
> 🦉 The `vitest/browser` types include the `page` and `userEvent` context as well as all the DOM matchers, like `.toBeVisible()`. You get them automatically in any test that imports from `vitest/browser`, but listing them in `types` makes them available everywhere.
