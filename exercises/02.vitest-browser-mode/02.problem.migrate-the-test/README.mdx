...
> 🦉 The biggest change when adopting Vitest Browser Mode is that it splits the rendering and the UI selection/interaction between two separate modules: the `vitest/browser` export of Vitest itself (for locating and interacting with elements) and a framework-specific renderer package, like `vitest-browser-react`.
