...
You can fix this by introducing _different projects_ for different types of tests. In fact, I think this is just the right task for you...
...
🐨 First, update <InlineFile file="vite.config.ts">`vite.config.ts`</InlineFile> to list multiple [projects](https://vitest.dev/guide/projects.html). Define one for unit tests and another for component tests.
...
🐨 Finally, rename the existing `file-preview.test.tsx` component test to `file-preview.browser.test.tsx` to be included in the correct Vitest project.
